...
namespace RPVoiceChat
{
    public class DebugSourceListDialog : GuiDialog
    {
        ...
        private GuiComposer Compose(KeyValuePair<string, DynamicAudioSource>[] keyValuePairs)
        {
            ...
            composer.AddButton(localSource.AudioOriginator.PlayerName, () => { api.ShowChatMessage("This is " + localSource.AudioOriginator.PlayerName + " and they are " + Enum.GetName(typeof(VoiceLevel), localSource.VoiceLevel)); return true; }, ElementBounds.Fixed(0, i, btnWidth, btnHeight));
            i += btnHeight;
            
            // Setup each player audio source beside local player
            foreach (var keyValuePair in keyValuePairs)
            {
                composer.AddButton(keyValuePair.Value.AudioOriginator.PlayerName, () => { api.ShowChatMessage("This is " + keyValuePair.Value.AudioOriginator.PlayerName + " and they are " + Enum.GetName(typeof(VoiceLevel), keyValuePair.Value.VoiceLevel).ToLower()); return true; }, ElementBounds.Fixed(0, i, btnWidth, btnHeight));
                i += btnHeight;
            }
